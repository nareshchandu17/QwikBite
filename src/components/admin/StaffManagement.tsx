...
const AddStaffCard: React.FC<AddStaffCardProps> = ({ onClick, className = '' }) => {
    return (
        <div 
            onClick={onClick}
            className={`bg-[rgba(20,20,20,0.6)]
  backdrop-blur-[24px] [-webkit-backdrop-filter:blur(24px)]
  border border-[rgba(255,255,255,0.08)]
  shadow-[0_8px_32px_0_rgba(0,0,0,0.3)] rounded-3xl p-6 flex flex-col items-center justify-center border-2 border-dashed border-white/10 hover:border-[#FF512F]/50 cursor-pointer transition-all group min-h-[360px] ${className}`}
        >
...
>
            ...
            {/* Hover Actions */}
            <div className="grid grid-cols-2 gap-3 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-2 group-hover:translate-y-0 relative z-10">
                <button 
...
const EditShiftModal: React.FC<EditShiftModalProps> = ({ staff, onClose, onSave }) => {
  ...
  backdrop-blur-[24px] [-webkit-backdrop-filter:blur(24px)]
  border border-[rgba(255,255,255,0.08)]
  shadow-[0_8px_32px_0_rgba(0,0,0,0.3)] w-full max-w-md rounded-3xl p-6 relative animate-scale-in border border-white/10 shadow-2xl">
...
const ViewProfileModal: React.FC<ViewProfileModalProps> = ({ staff, onClose }) => {
  ...
  backdrop-blur-[24px] [-webkit-backdrop-filter:blur(24px)]
  border border-[rgba(255,255,255,0.08)]
  shadow-[0_8px_32px_0_rgba(0,0,0,0.3)] w-full max-w-lg rounded-3xl overflow-hidden relative animate-scale-in border border-white/10 shadow-2xl">
                {/* Profile Header */}
...
interface AddStaffModalProps {
    onClose: () => void;
    onAdd: (staff: Omit<BaseStaff, '_id'>) => void;
}
...
const AddStaffModal: React.FC<AddStaffModalProps> = ({ onClose, onAdd }) => {
    const [formData, setFormData] = useState<Omit<BaseStaff, '_id'>>({
        ...
        contact: '',
        performance: 100, // Default to max for new hires
        createdAt: new Date(),
        updatedAt: new Date()
    });
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!formData.name?.trim() || !formData.email?.trim() || !formData.contact?.trim()) {
            toast.error('Please fill in all required fields: Name, Email, and Contact');
            return;
        }
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(data.details || data.error || 'Failed to add staff');
            }
            
            // Call onAdd with the new staff data
            const newStaff = {
                ...formData,
                name: formData.name.trim(),
                email: email,
            ...
            // Use type assertion to include _id
            onAdd({
                ...newStaff,
                _id: data.data._id
            } as Staff);
            
            toast.success(`Staff ${formData.name.trim()} added successfully!`);
        } catch (error) {
            ...
            if (error instanceof Error) {
                ...
                if (error.message.includes('not a valid role')) {
                    errorMessage = 'Please select a valid role from the list.';
                    ...
                    errorMessage = 'Please check the form for errors. All fields are required.';
                }
            }
  ...
  backdrop-blur-[24px] [-webkit-backdrop-filter:blur(24px)]
  border border-[rgba(255,255,255,0.08)]
  shadow-[0_8px_32px_0_rgba(0,0,0,0.3)] w-full max-w-2xl overflow-hidden rounded-3xl flex flex-col relative animate-scale-in shadow-2xl shadow-black/50 border border-white/10">
...
const StaffManagement: React.FC = () => {
  ...
  className="
    ...
    hover:shadow-[0_12px_40px_rgba(255,81,47,0.25),0_0_30px_rgba(240,152,25,0.2)]
    hover:border-[rgba(255,81,47,0.35)]
  "
>
  ...
  backdrop-blur-[24px] [-webkit-backdrop-filter:blur(24px)]
  border border-[rgba(255,255,255,0.08)]
  shadow-[0_8px_32px_0_rgba(0,0,0,0.3)] p-4 rounded-xl transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)]
    hover:-translate-y-[3px] hover:scale-[1.015]
